...
def safe_convert_to_int(value):
    try:
        ...
        print(f"Warning: Failed to convert {value} to int")
        return None
...
if __name__ == '__main__':
    ...
    if args.phase != 'infer':
        ...
        train_ids = list(map(lambda x: safe_convert_to_int(x[-10:-4]), train_ids))
        train_ids = [id for id in train_ids if id is not None]  # Remove None values
