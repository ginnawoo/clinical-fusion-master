...
if __name__ == '__main__':
    ...
    if args.phase != 'infer':
        ...
        model = Doc2Vec(dm=0, vector_size=200, negative=5,
                        alpha=0.025, hs=0, min_count=5, sample=0, workers=16)
        model.build_vocab([x for x in train_tagged])
        for epoch in tqdm(range(epochs)):
            model.train(shuffle([x for x in train_tagged]),
        ...
        df['vector'] = df['text'].apply(
            lambda note: doc2vec.infer_vector(note).tolist())
        df = df.groupby('hadm_id')['vector'].apply(list).reset_index()
        vector_dict = {}
        for idx, row in df.iterrows():
            vector_dict[str(int(row['hadm_id']))] = row['vector']
        json.dump(vector_dict, open(
            './data/processed/files/vector_dict.json', 'w'))
